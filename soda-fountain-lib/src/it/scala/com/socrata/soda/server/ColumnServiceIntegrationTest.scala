...
object ColumnServiceIntegrationTest {
  val rn = "soda-column-service-int-test-1"
}
...
class ColumnServiceIntegrationTest extends IntegrationTest with ColumnServiceIntegrationTestFixture {
  ...
  test("column service getSchema") {
    ...
    jsonCompare(gResponse.getResponseBody, "{ verify:'column schema'}")
  }
  test("column service - add/drop column") {
    ...
    d1Response.getStatusCode must equal (204)
    ...
    //add column
    val pResponse = dispatch("POST", "dataset", Some(ColumnServiceIntegrationTest.rn), Some(id), None,  None)
