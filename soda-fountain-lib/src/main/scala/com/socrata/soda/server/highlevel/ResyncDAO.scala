...
object ResyncDAO {
  ...
  case class DatasetNotInSecondary(secondary: SecondaryId, dataset: ResourceName) extends ResyncResponse
...
case class ResyncDAOImpl(store: NameAndSchemaStore, dc: DataCoordinatorClient) extends ResyncDAO {
  override def resync(resourceName: ResourceName, secondary: SecondaryId): ResyncDAO.ResyncResponse = {
    ...
    result match {
      ...
      case Some(DatasetNotInSecondaryResult(secondary)) => ResyncDAO.DatasetNotInSecondary(secondary, resourceName)
