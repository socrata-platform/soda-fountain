...
import com.socrata.soda.message.{GroupReplicationComplete, MessageProducerFromConfig, ViewUid}
...
class SodaFountain(config: SodaFountainConfig) extends Closeable {
  ...
  val msg = GroupReplicationComplete(ViewUid("xx"), "groupName", Set("pg"), 1, 1)
