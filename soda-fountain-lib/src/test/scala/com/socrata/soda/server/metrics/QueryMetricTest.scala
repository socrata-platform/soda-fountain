...
private class QueryOnlyRowDAO(testDatasets: Set[TestDataset]) extends RowDAO {
  ...
  def upsert[T](user: String, datasetRecord: DatasetRecordLike, data: Iterator[RowUpdate], reqId: String, rowUpdateOption: RowUpdateOptionChange)(f: UpsertResult => T): T = ???
